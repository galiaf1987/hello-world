			...
			<ul <?if(!($index%3)):?>class="last"<?endif;?>>
				...
				<?if($arItems["SECTIONS"]):?>
					...
					<?foreach($arItems["SECTIONS"] as $arItem ):?>
						<li  <?=($i > 40 ? 'class="d menu_item" style="display: none;"' : 'class="menu_item"')?>><a href="<?=$arItem["SECTION_PAGE_URL"]?>" <?=($arUrl[0]==$arItem["SECTION_PAGE_URL"] ? "class='current'" : "")?>><?=$arItem["NAME"]?></a></li>
						<?++$i;?>
					<?endforeach;?>
					<?if(count($arItems["SECTIONS"] ) > 41 ):?>
						<!--noindex-->
						<li class="see_more">
