...
//echo '<pre>';var_dump($arResult["SECTION"]);echo '</pre>';

$arResult["UF_DELIVERY"] = 500;
if($arResult["CATALOG_QUANTITY"]>0) {
	$arResult["UF_DELIVERY_SROK"] = '0-2';
}

$sectionsPath = DXOpt::getNavChain($arResult["SECTION"]);
foreach($sectionsPath as $arSection) {
	if($arSection["UF_DELIVERY"]!==null) {
		$arResult["UF_DELIVERY"] = $arSection["UF_DELIVERY"];
	}
	if($arSection["UF_DELIVERY_SROK"]!==null && preg_match('@\d+(-\d+)?@', $arSection["UF_DELIVERY_SROK"], $m)) {
		$arResult["UF_DELIVERY_SROK"] = $m[0];
	}
	if($arSection['UF_BASKET_DISCOUNT']) {
		$arResult['UF_BASKET_DISCOUNT'] = $arSection['UF_BASKET_DISCOUNT'];
	}
	if($arSection['UF_MOUNTING']) {
		$arResult['UF_MOUNTING'] = $arSection['UF_MOUNTING'];
	}
}

if($arResult["UF_DELIVERY_SROK"]=="0") {
	$arResult["UF_DELIVERY_SROK"] = 'сегодня';
} elseif(preg_match('@0-(\d+)@', $arResult["UF_DELIVERY_SROK"], $m)) {
	$arResult["UF_DELIVERY_SROK"] = 'до '.$m[1].' дней';
} elseif(preg_match('@\d+-\d+@', $arResult["UF_DELIVERY_SROK"])) {
	$arResult["UF_DELIVERY_SROK"] = $arResult["UF_DELIVERY_SROK"].' дня';
} elseif(preg_match('@\d+@', $arResult["UF_DELIVERY_SROK"])) {
	$arResult["UF_DELIVERY_SROK"] = $arResult["UF_DELIVERY_SROK"].' дня';
}
