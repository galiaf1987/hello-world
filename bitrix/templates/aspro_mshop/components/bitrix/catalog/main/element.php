...
elseif(strlen(trim($arResult["VARIABLES"]["SECTION_CODE"])) > 0){
	$section=CMshopCache::CIBlockSection_GetList(array('CACHE' => array("MULTI" =>"N", "TAG" => CMshopCache::GetIBlockCacheTag($arParams["IBLOCK_ID"]))), array('IBLOCK_ID'=>$arParams['IBLOCK_ID'], 'GLOBAL_ACTIVE' => 'Y', "=CODE" => $arResult["VARIABLES"]["SECTION_CODE"]), false, array("ID"));
}
if(!$section["ID"]){
...
$cachePath = '/'.SITE_ID.'/accesories';
$timeSeconds = 600;
$cacheId = $MShopSectionID;

if( $obCache->InitCache($timeSeconds, $cacheId, $cachePath) ) {
	$arExpandable = $obCache->GetVars();
} elseif( $obCache->StartDataCache()  ) {
	global $CACHE_MANAGER;
	$CACHE_MANAGER->StartTagCache($cachePath);

	$arExpandable = array();;
	$rs = CIBlockSection::GetNavChain($arParams["IBLOCK_ID"], $MShopSectionID);
	while($ar = $rs->Fetch()) {
		$rs2 = CIBlockSection::GetList(array(), array("IBLOCK_ID"=>$arParams["IBLOCK_ID"], "ID"=>$ar["ID"]), false, array("ID", "UF_ACCESORIES"));
		if($ar2 = $rs2->Fetch()) {
			if($ar2["UF_ACCESORIES"]) {
				$arExpandable=array_merge($arExpandable, $ar2["UF_ACCESORIES"]);
			}
		}
	}
	...
	$arExpandable = array_unique($arExpandable);
	$obCache->EndDataCache($arExpandable);
}

?>
<div itemscope itemtype="http://schema.org/Product" class="catalog_detail">
...
if($arExpValues){
	$arAllValues["EXPANDABLES"]=array_merge($arExpandable, $arExpValues);
} else if($arExpandable) {
	$arExpValues = true;
	$arAllValues["EXPANDABLES"]=$arExpandable;
}
/*accessories goods*/
