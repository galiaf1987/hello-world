...
<?if($iSectionsCount > 0):?>
	...
	if($values['SECTION_META_KEYWORDS']){
		...
		<?$APPLICATION->IncludeComponent(
			...
			"subsections_list.new", //$_SERVER["REMOTE_ADDR"] == "31.130.148.28" ? "subsections_list.new" : "subsections_list",
			Array(
		...
		<?if($posSectionDescr=="BOTTOM"){?>
			...
			<?if ($arSection["SEO_DESCRIPTION"]):?>
				<div class="group_description_block bottom clearfix">
					<div><?=$arSection["SEO_DESCRIPTION"]?></div>
				</div>
			<?elseif($arSection["DESCRIPTION"]):?>
				<div class="group_description_block top clearfix">
			...
			<?}?>
				<?$APPLICATION->IncludeComponent(
					...
					Array(
						...
						"ELEMENT_SORT_FIELD2" => 'SORT',//$arParams["ELEMENT_SORT_FIELD2"],
						"ELEMENT_SORT_ORDER2" => 'ASC',//$arParams["ELEMENT_SORT_ORDER2"],
