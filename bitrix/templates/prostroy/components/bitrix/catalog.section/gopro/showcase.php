			...
			?><div class="inner"><?
					...
					?><div class="soloprice"><?
						foreach($arResult['PRICES'] as $PRICE_CODE => $arResPrice){
							if(!$arResult['PRICES'][$PRICE_CODE]['CAN_VIEW'])
								continue;
							$arPrice = $PRODUCT['PRICES'][$PRICE_CODE];
							?><span class="price gen price_pdv_<?=$PRICE_CODE?>"><?=GetMessage('GOPRO_SHOWCASE_PRICE')?> <?=$arPrice['PRINT_DISCOUNT_VALUE']?></span><?
					...
					?></div><?
						...
						?><noindex><div class="buy"><?
							?><form class="add2basketform js-buyform<?=$arItem['ID']?> js-synchro<?if(!$PRODUCT['CAN_BUY']):?> cantbuy<?endif;?> clearfix" name="add2basketform"><?
								...
								?><a rel="nofollow" class="submit add2basket" href="#" title="<?=GetMessage('ADD2BASKET')?>"><?=GetMessage('CT_BCE_CATALOG_BUY')?></a><?
				...
				if($arParams['OFF_SMALLPOPUP']!='Y') {
					?><div class="popup padd"><?
						...
						if($arParams['USE_STORE']=='Y') { ?>
							<div>
								...
								<? if($arParams['USE_MIN_AMOUNT']=='Y') { ?>
									...
									<? } ?>
								<? } else { ?>
									<span><?=$arItem['FULL_CATALOG_QUANTITY'];?></span>
								<? } ?>
							</div>
						<? }
						// DESCRIPTION
						...
						// FAVORITE & SHARE
						?><div class="favorishare clearfix"><!--noindex--><?
							if($arParams['USE_FAVORITE']=='Y' || $arParams['USE_SHARE']=='Y')
							{
								...
								if($arParams['USE_SHARE']=='Y')
								{
									...
									?><div class="share"><?
										?><span class="b-share"><a rel="nofollow" class="fancyajax fancybox.ajax email2friend b-share__handle b-share__link b-share-btn__vkontakte" href="<?=SITE_DIR?>email2friend/?link=<?=CUtil::JSUrlEscape($protocol.$_SERVER['HTTP_HOST'].$arItem['DETAIL_PAGE_URL'])?>" title="<?=GetMessage('EMAIL2FRIEND')?>"><i class="b-share-icon icon pngicons"></i></a></span><?
										?><span id="detailYaShare_<?=$arItem['ID']?>"></span><?
										?><script type="text/javascript">
										new Ya.share({
											link: '<?=$protocol . $_SERVER['HTTP_HOST'] . $arItem['DETAIL_PAGE_URL']?>',
