...
if( is_array($arResult['ITEMS']) && count($arResult['ITEMS'])>0 ) {
	foreach($arResult['ITEMS'] as $key1 => $arItem) {
		...
		if($arParams["SECTION_ID"]!=$arItem["~IBLOCK_SECTION_ID"]) {
			$rs = CIBlockElement::GetList(array(), array("IBLOCK_ID"=>$arParams["IBLOCK_ID"], "ID"=>$arItem["ID"]), false, array("nTopCount"=>1), array("DETAIL_PAGE_URL"));
			$ar = $rs->GetNext();
			$arResult['ITEMS'][$key1]["DETAIL_PAGE_URL"] = $ar["DETAIL_PAGE_URL"];
		}
	}
}
// /QB and DA2
