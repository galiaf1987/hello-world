	...
	?><div class="part items clearfix"><?
		...
		?><div class="btns clearfix"><?
			...
			?><span class="totaltext">
				<? if(floatval($arResult['allWeight']) > 0) { ?>
					<span class="name"><?=GetMessage('SALE_TOTAL_WEIGHT')?></span>
					<span class="take_normalCount"><?=$arResult['allWeight_FORMATED']?></span>
				<? } ?>
				<? if(floatval($arResult['DISCOUNT_PRICE_ALL']) > 0) { ?>
					<span class="name"><?=GetMessage('SALE_DISCOUNT')?>:</span>
					<span class="take_normalCount"><?=$arResult['DISCOUNT_PRICE_ALL_FORMATED']?></span>
				<? } ?>
				<span class="name"><?=GetMessage('SALE_EC_HEADER_LINK_PRODS')?>:</span>
				<span class="take_normalCount"><?=$normalCount?></span>
				<span class="name"><?=GetMessage('SALE_SUM')?>:</span>
				<span class="take_allSum_FORMATED"><?=$arResult['allSum_FORMATED']?></span>
			</span><?
			?><div class="clear"></div><?
