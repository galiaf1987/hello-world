...
?>
	<div class="brandslist clearfix mainstyles">
		<div class="title"><h1><a href="/brands/">Бренды</a></h1></div>
		<div class="item bukva">
		<? foreach ($arResult['rows'] as $row): ?>
			<?
			$url = str_replace(
				array('#ID#', '#BLOCK_ID#', '#XML_ID#'),
				array($finalValue, intval($arParams['BLOCK_ID']), $row['UF_XML_ID']),
				$arParams['DETAIL_URL']
			);
			?>
			<a href="<?=htmlspecialcharsbx($url)?>"><?=$row['UF_NAME']?></a>
		<? endforeach; ?>
		</div>
	</div>
