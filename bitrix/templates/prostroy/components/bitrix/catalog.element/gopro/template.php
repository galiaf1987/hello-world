...
?><div itemscope itemtype="http://schema.org/Product" class="elementdetail js-element js-elementid<?=$arResult['ID']?> <?if($HAVE_OFFERS):?>offers<?else:?>simple<?endif;?><?
	...
	?><div class="pictures changegenimage"><?
		?><div class="pic"><?
			...
			{
				?><div class="glass"><?
					?><img itemprop="image" class="js_picture_glass genimage" src="<?=$arResult['FIRST_PIC_DETAIL']['SRC']?>" alt="<?=$arResult['FIRST_PIC_DETAIL']['RESIZE']['ALT']?>" title="<?=$arResult['FIRST_PIC_DETAIL']['RESIZE']['TITLE']?>" /><?
	...
	<div itemprop="additionalProperty" itemscope itemtype="http://schema.org/PropertyValue" class="properties">
		<? foreach ($arResult['CHARACTERISTICS'] as $key => $property): ?>
			<div class="property <? if ($key % 2 === 0) : ?>property-even<? endif; ?>">
				<span itemprop="name" class="name">
					<?= $property['NAME'] ?>
				</span>
				<span itemprop="value" class="value">
	...
	<div class="info">
			...
			?><div class="soloprice"><?
				...
				foreach($arResult['CAT_PRICES'] as $PRICE_CODE => $arResPrice)
				{
					if(!$arResult['CAT_PRICES'][$PRICE_CODE]['CAN_VIEW'])
						continue;
					$arPrice = $PRODUCT['PRICES'][$PRICE_CODE];
					?><table class="price" itemprop="offers" itemscope itemtype="http://schema.org/Offer"><?
						?><tr><?
							?><td><div class="line"><span class="name"><?=GetMessage('SOLOPRICE_PRICE')?><span></div></td><td class="nowrap"><?
							?><span class="price<?if( $arPrice['DISCOUNT_DIFF']>0 ):?> new<?endif;?> gen price_pdv_<?=$PRICE_CODE?>"><?=$arPrice['PRINT_DISCOUNT_VALUE']?></span><?
							?><meta itemprop="price" content="<?=$arPrice['DISCOUNT_VALUE']?>"><meta itemprop="priceCurrency" content="RUB"><?
							?></td><?
						?></tr><?
						if( $arPrice['DISCOUNT_DIFF']>0 )
						{
						...
						if($arResult["PROPERTIES"]["PRICE_OPT"]["VALUE"]>0) {
							?><td><div class="line"><span class="name">Оптовая цена<span></div></td><td class="nowrap"><span class="price gen price_pdv_BASE"><?=CCurrencyLang::CurrencyFormat($arResult["PROPERTIES"]["PRICE_OPT"]["VALUE"], 'RUB')?></span></td><?
						}
						// STORES
						?><tr><td><?
							?><div class="line"><span class="name">Наличие</span></div><?
						?></td><td class="nowrap"><?
							?><?
								preg_match('@<a[^>]*>(.*?)</a>@ims', $store, $m);
								if(preg_match('@есть@iu', $m[1])) {
				...
				?><?if(!in_array($arResult["SECTION"]["PATH"][0]["ID"], array(249,530,561))):?>
                <div class="delivery">
                    <table>
                        <tbody>
                <?if(isset($arPrice['DISCOUNT_VALUE']) && $arPrice['DISCOUNT_VALUE'] > 3500):?>
                        <tr><td><div class="line"><span class="name">Доставка по Москве<span></span></span></div></td><td class="nowrap"><span class="price gen">0 руб.</span></td></tr>
                <?endif;?>
                        <tr><td><div class="line"><span class="name">Монтаж</span></div></td><td class="nowrap"><span class="price gen">от 3000 руб.</span></td></tr>
                        </tbody>
                    </table></div><?
				?><?endif;?><?
				?></div><?
				?><a href="/payment/"><img src="<?=SITE_TEMPLATE_PATH?>/img/paysystems.png" alt="Способы оплаты" class="paysystems" /></a><?
			?></div><?
		}
		// PROPERTIES
		...
		?><noindex><div class="buy clearfix"><?
			?><form class="add2basketform js-buyform<?=$arResult['ID']?> js-synchro<?if(!$PRODUCT['CAN_BUY']):?> cantbuy<?endif;?> clearfix" name="add2basketform"><?
				...
				?><a rel="nofollow" class="submit add2basket" href="#" title="<?=GetMessage('ADD2BASKET')?>"><i class="icon pngicons"></i><?=GetMessage('CT_BCE_CATALOG_BUY')?></a><?
				...
				?><a rel="nofollow" class="discount" href="/action/skidki_na_radiatory_rifar/" title=""><?/*<i class="icon pngicons"></i>*/?>Скидки</a><?
				/*
		...
		$tmp = array_reverse($arResult['SECTION']['PATH']);
		$arSkidka = CCatalogProductProviderOtp::getDiscountList();
		foreach($tmp as $p) {
			if($arSkidka[$p["ID"]]) {
				?><div class="soloprice delivery" data-discount="<?=htmlspecialchars(json_encode($arSkidka[$p["ID"]]))?>" data-price="<?=$arPrice["DISCOUNT_VALUE"]?>"><?
					?><table><tr><td><div class="line"><span class="name"><?
						?>Цена при заказе <span class="n">1</span> шт.<?
					?></span></div></td><td><?
						?><span class="gen price"><span class="prc"><?=$arPrice["DISCOUNT_VALUE"]?></span> руб.</span><?
					?></td></tr><tr><td><div class="line"><span class="name"><?
						?>Итого при заказе <span class="n">1</span> шт.<?
					?></span></div></td><td><?
						?><span class="gen price"><span class="itog"><?=$arPrice["DISCOUNT_VALUE"]?></span> руб.</span><?
					?></td></tr></table><?
				?></div><?
				break;
			}
		...
		if($arResult['PROPERTIES']['WBP']['VALUE']) {
			?><br /><br /><p class="wbp"><a href="/action/garantiya_luchshey_tseny/"><img src="<?=SITE_TEMPLATE_PATH?>/img/wbp.jpg" alt="гарантия лучшей цены" /></a></p><?
		}
		// SHARE
...
/*				?><span id="detailYaShare_<?=$arResult['ID']?>"></span><?
	...
	?>
			...
			<div class="section">
				<h3>Выберите подарок к этому радиатору</h3>
				<div class="gifts">
				<? foreach ($arResult['GIFTS'] as $gift) : ?>
					<a class="gift" href="<?=$gift['DETAIL_PAGE_URL']?>" target="_blank">
						<i class="icon"></i><span class="name"><?=$gift['NAME']?></span>
					</a>
				<? endforeach; ?>
				</div>
			</div>
			<? endif;?>
			<div class="share-social"><div class="vcenter">Поделиться: </div>
		...
		?><div class="contentbody clearfix"><?
			?><div class="contentinner"><?
				...
				$APPLICATION->IncludeComponent('redsign:grupper.list',
					'gopro',
					array(
						'DISPLAY_PROPERTIES' => $arResult['CHARACTERISTICS'],//array_diff_key($arResult['DISPLAY_PROPERTIES'], $arTemp),
						'CACHE_TIME' => 36000,
					),
					$component,
					array('HIDE_ICONS'=>'Y')
			...
			?><div class="content files selected" id="prop<?=$sPropCode?>"><?
				...
				?><div class="contentbody clearfix"><?
					?><div class="contentinner"><?
						...
						{
							?><a class="docs" target="_blank" href="<?=$arFile['FULL_PATH']?>"><?
...
<?
	$path = array();
	foreach($arResult["SECTION"]["PATH"] as $ar) {
		$path[]=$ar['NAME'];
	}
?>
try {
			...
			"products": [
				{
					"id": "<?=$arResult["ID"]?>",
					"name" : "<?=$arResult["NAME"]?>",
					"price": <? echo $arResult['MIN_PRICE']['DISCOUNT_VALUE']; ?>,
<?if($manufacturer):?>
					"brand": "<?=$manufacturer?>",
<?endif;?>
...
try {
  gtag('event', 'page_view', {
    'send_to': 'AW-830679255',
    'ecomm_pagetype': 'detail',
    'ecomm_prodid': '<?=$arResult["ID"]?>',
    'ecomm_totalvalue': '<?=CCurrencyRates::ConvertCurrency($arResult["MIN_PRICE"]["VALUE"], $arResult["MIN_PRICE"]["CURRENCY"], 'RUB')?>'
