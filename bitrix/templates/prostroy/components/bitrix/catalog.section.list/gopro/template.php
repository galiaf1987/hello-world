				...
				if($arSection["DEPTH_LEVEL"] == $arResult['LEVELING']['FIRST_LEVEL'])
				{
					...
					?><li class="section" id="<?=$this->GetEditAreaId($arSection["ID"]);?>"><?
						...
						if( ($arSection["RIGHT_MARGIN"]-$arSection["LEFT_MARGIN"])>1 && $arParams['SHOW_COUNT_LVL2']>0 && $arParams['TOP_DEPTH']>1 ) // is_parent
						{
							...
							?><a class="dop" href="<?=$arSection["SECTION_PAGE_URL"]?>">
								<span class="cnt">В наличии: <span><?=$arSection["CNT"]?></span></span>
								<?if($arSection["MIN_PRICE"]):?><span class="prc">Цена от <span><?=CCurrencyLang::CurrencyFormat($arSection["MIN_PRICE"], "RUB")?></span></span><?endif;?>
							</a><?
						}
					$index1++;
					$index2 = 1;
