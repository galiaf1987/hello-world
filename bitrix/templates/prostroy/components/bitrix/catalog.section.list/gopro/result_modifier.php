...
if(is_array($arResult['SECTIONS']) && count($arResult['SECTIONS'])){
	...
	foreach($arResult['SECTIONS'] as $iSectionKey => &$arSection){
		$rs = CIBlockElement::GetList(array(), array("IBLOCK_ID"=>$arParams["IBLOCK_ID"], "SECTION_ID"=>$arSection["ID"], "!ACTIVE"=>"N", "!SECTION_GLOBAL_ACTIVE"=>"N", "INCLUDE_SUBSECTIONS"=>"Y"));
		$arSection["CNT"] = $rs->SelectedRowsCount();
	}
	foreach($arResult['SECTIONS'] as $iSectionKey => &$arSection){
		$rs = CIBlockElement::GetList(array("CATALOG_PRICE_1"=>"ASC"), array("IBLOCK_ID"=>$arParams["IBLOCK_ID"], "SECTION_ID"=>$arSection["ID"], "!ACTIVE"=>"N", "!SECTION_GLOBAL_ACTIVE"=>"N", "INCLUDE_SUBSECTIONS"=>"Y", ">CATALOG_PRICE_1"=>0), false, false, array("ID", "CATALOG_PRICE_1", "CATALOG_CURRENCY_1"));
		while($ar = $rs->GetNext()) {
			
			$prc = CCurrencyRates::ConvertCurrency($ar["CATALOG_PRICE_1"], $ar["CATALOG_CURRENCY_1"], "RUB"); 
			if(!isset($arSection["MIN_PRICE"]) || $prc<$arSection["MIN_PRICE"]) {
				$arSection["MIN_PRICE"] = $prc;
			}
		}
	}
}

/* moved to component_epilog 
