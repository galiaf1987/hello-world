...
if( \Bitrix\Main\Loader::includeModule('iblock') )
{
	...
	if(\Bitrix\Main\Loader::includeModule('iblock'))
	{
		...
		if($obCache->InitCache(36000, serialize($arFilter) ,'/iblock/catalog'))
		{
			...
			$arCurSection = array();
			$dbRes = CIBlockSection::GetList(array(), $arFilter, false, array('ID','PICTURE', 'LEFT_MARGIN','RIGHT_MARGIN', 'NAME', 'IBLOCK_SECTION_ID', 'SECTION_PAGE_URL', 'DESCRIPTION', 'UF_H1', 'UF_TITLE', 'UF_KEYWORDS', 'UF_DESCRIPTION'));
			...
			$dbRes = CIBlockSection::GetList(array(), $arFilter, false, array('ID', 'LEFT_MARGIN', 'RIGHT_MARGIN', 'NAME'), array('nTopCount'=>1));
			$arCurSection['HAVE_SUBCATS'] = $dbRes->Fetch();
			...
			$arCurSection['HAVE_PRODUCTS'] = $dbRes->Fetch();
...
if($arCurSection['PICTURE']) {
	$arImage = CFile::GetFileArray($arCurSection['PICTURE']);
	$APPLICATION->SetPageProperty('og:image', $arImage['SRC']);
}
	...
	if( $arParams['SECTIONS_VIEW_MODE']=='VIEW_SECTIONS' && ( ($arCurSection['RIGHT_MARGIN']-$arCurSection['LEFT_MARGIN'])>1 ) )
	{
		...
		$APPLICATION->IncludeComponent(
			...
			Array(
				...
				/*'SECTION_ID' => $arResult['VARIABLES']['SECTION_ID'],
				'SECTION_CODE' => $arResult['VARIABLES']['SECTION_CODE'],*/
		'SECTION_ID' => $arCurSection['HAVE_SUBCATS']||$arCurSection['HAVE_PRODUCTS']?$arCurSection['IBLOCK_SECTION_ID']:$arResult['VARIABLES']['SECTION_ID'],
		'SECTION_CODE' => $arCurSection['HAVE_SUBCATS']||$arCurSection['HAVE_PRODUCTS']?'':$arResult['VARIABLES']['SECTION_CODE'],
				//'CACHE_TYPE' => $arParams['CACHE_TYPE'],
		'CACHE_TYPE' => 'N',
				'CACHE_TIME' => $arParams['CACHE_TIME'],
				'CACHE_GROUPS' => $arParams['CACHE_GROUPS'],
		...
		if($arCurSection['HAVE_SUBCATS']||$arCurSection['HAVE_PRODUCTS']) {
			//$APPLICATION->AddChainItem($arCurSection['NAME'], $arCurSection['SECTION_PAGE_URL']);
		}

		//var_dump($arCurSection['IBLOCK_SECTION_ID'],$arResult['VARIABLES']['SECTION_ID'],$arResult['VARIABLES']['SECTION_CODE']);
...
if(/*$arCurSection['HAVE_SUBCATS'] &&*/ !isset($_GET['set_filter'])) {
		$APPLICATION->IncludeComponent(
			'bitrix:catalog.section.list',
			'gopro',
			array(
		...
		if($arCurSection["UF_H1"]) $APPLICATION->SetTitle($arCurSection["UF_H1"]);
		if($arCurSection["UF_TITLE"]) $APPLICATION->SetPageProperty('title', $arCurSection["UF_TITLE"]);
		if($arCurSection["UF_KEYWORDS"]) $APPLICATION->SetPageProperty('keywords', $arCurSection["UF_KEYWORDS"]);
		if($arCurSection["UF_DESCRIPTION"]) $APPLICATION->SetPageProperty('description', $arCurSection["UF_DESCRIPTION"]);
}

if($arCurSection['HAVE_PRODUCTS'] || isset($_GET['set_filter'])) {
...
$arOutupVariables = array(0 => "10",1 => "15",2 => "20",3 => "30");
...
$APPLICATION->IncludeComponent(
	...
	array(
		...
		"ALFA_OUTPUT_OF_DEFAULT" => ( isset($arParams["SORTER_OUTPUT_OF_DEFAULT"]) ? $arParams["SORTER_OUTPUT_OF_DEFAULT"] : '30' ),
	...
	$JSON['TYPE'] = 'OK';
				...
				?><?$intSectionID = $APPLICATION->IncludeComponent(
					...
					array(
						...
						// columns
						'COLUMNS5' => 'N',
						'ADD_SECTIONS_CHAIN' => isset($_GET['set_filter'])?'Y':'N'
					),
					$component,
					array('HIDE_ICONS'=>'Y')
				);?><?
...
$APPLICATION->IncludeComponent('redsign:catalog.sorter', 'gopro', array(
	...
	false
		...
		?><div class="catalog_section_description"><?
			if(!isset($_REQUEST["PAGEN_1"])) echo $arCurSection["DESCRIPTION"];
		?></div><?
		?></div><?
if( $IS_AJAX_CATALOG )
{
