...
/*if(true||$_SERVER["REMOTE_ADDR"]=="77.50.156.6"):
?><div class="viewvwedproducts"><?
include 'viewedproducts.php';
?></div><?
endif;*/
...
<?
	$obCache = new CPHPCache();
	if($obCache->InitCache(36000, $ElementID ,'/iblock/analog')) {
		$arCur = $obCache->GetVars();
	} elseif($obCache->StartDataCache()) {
		$arCur = array();
		$rsCurProd = CIBlockElement::GetList(array(), array('IBLOCK_ID' => $arParams['IBLOCK_ID'], 'ID' => $ElementID), false, array("nTopCount" => 1), array("IBLOCK_SECTION_ID"));
		$arCur["EL"] = $rsCurProd->GetNext();
		$arCur["SC"] = CIBlockSection::GetByID($arCur["EL"]["IBLOCK_SECTION_ID"])->GetNext();
		if(defined('BX_COMP_MANAGED_CACHE')) {
			global $CACHE_MANAGER;
			$CACHE_MANAGER->StartTagCache('/iblock/catalog');
			if($arCur) {
		...
		$obCache->EndDataCache($arCur);
	}
?>
<?if($arCur["SC"]):
	global $analogFilter;
	$analogFilter = array("!ID"=>$ElementID, "SECTION_ID"=>$arCur["SC"]["ID"]);
	?>
	<?$intSectionID = $APPLICATION->IncludeComponent(
		...
		array(
			...
			'VIEW' => 'showcase',
			'CLASS' => 'analog',
			'TITLE' => 'Доступные модели '.$arCur["SC"]["NAME"]
		),
		$component,
		array('HIDE_ICONS'=>'Y')
	);?>
...
<?
	$APPLICATION->SetAdditionalCSS(SITE_TEMPLATE_PATH.'/js/jcarousel/jcarousel.responsive.css');
	$APPLICATION->SetAdditionalCSS(SITE_TEMPLATE_PATH.'/components/bitrix/catalog.section/light/style.css');
	$APPLICATION->AddHeadScript(SITE_TEMPLATE_PATH.'/js/jcarousel/jquery.jcarousel.min.js');
	$APPLICATION->AddHeadScript(SITE_TEMPLATE_PATH.'/js/jcarousel/jcarousel.responsive.js');
?>
<?endif;?>
