...
if (!function_exists("yandex_text2xml"))
{
	...
	function yandex_text2xml_utf8($text, $bHSC = false, $bDblQuote = false)
	{
		global $APPLICATION;
...
if (empty($arRunErrors))
{
	...
	$fp2 = @fopen($_SERVER["DOCUMENT_ROOT"].str_replace('yandex', 'google', $SETUP_FILE_NAME).'-tmp', "wb");

	if (!$fp = @fopen($_SERVER["DOCUMENT_ROOT"].$SETUP_FILE_NAME.'-tmp', "wb"))
	{
...
if (empty($arRunErrors))
{
	...
	fwrite($fp2, '<?header("Content-Type: text/xml; charset=utf-8");'."\n");
	fwrite($fp2, 'echo "<"."?xml version=\"1.0\"?".">\n";?>');
	fwrite($fp2, '<rss xmlns:g="http://base.google.com/ns/1.0" version="2.0">'."\n");
	fwrite($fp2, '	<shop>'."\n");
	fwrite($fp2, '		<name>'.COption::GetOptionString('main', 'site_name', '').'</name>'."\n");
	fwrite($fp2, '		<url>'.$usedProtocol.htmlspecialcharsbx($ar_iblock['SERVER_NAME']).'</url>'."\n");
	fwrite($fp2, '		<company>'.COption::GetOptionString('main', 'site_name', '')."</company>\n");
	fwrite($fp2, '	<channel>'."\n");
	...
	if (!$bAllSections)
	{
		...
		{
			$sectionIterator = CIBlockSection::GetNavChain($IBLOCK_ID, $arSections[$i], array('ID', 'IBLOCK_SECTION_ID', 'NAME', 'LEFT_MARGIN', 'RIGHT_MARGIN', 'UF_DELIVERY', 'UF_DELIVERY_SROK', 'UF_PRICE_KOEF', 'UF_SALES_NOTES', 'UF_GIFT'));
			$curLEFT_MARGIN = 0;
			$curRIGHT_MARGIN = 0;
			while ($tsection = $sectionIterator->Fetch())
			{
				$section = CIBlockSection::GetList(array(), array('IBLOCK_ID'=>$IBLOCK_ID, 'ID'=>$tsection['ID']), false, array('ID', 'IBLOCK_SECTION_ID', 'NAME', 'LEFT_MARGIN', 'RIGHT_MARGIN', 'UF_DELIVERY', 'UF_DELIVERY_SROK', 'UF_PRICE_KOEF', 'UF_SALES_NOTES', 'UF_GIFT'))->Fetch();
				...
				$arAvailGroups[$section['ID']] = array(
					...
					'NAME' => $section['NAME'],
					'UF_DELIVERY' => $section['UF_DELIVERY'],
					'UF_DELIVERY_SROK' => $section['UF_DELIVERY_SROK'],
					'UF_PRICE_KOEF' => $section['UF_PRICE_KOEF'],
					'UF_SALES_NOTES' => $section['UF_SALES_NOTES'],
				);
				if($section['UF_GIFT']) {
					$arGifts[$section['ID']] = $section['UF_GIFT'];
				}
			...
			$sectionIterator = CIBlockSection::GetList(array("LEFT_MARGIN"=>"ASC"), $filter, false, array('ID', 'IBLOCK_SECTION_ID', 'NAME', 'UF_DELIVERY', 'UF_DELIVERY_SROK', 'UF_PRICE_KOEF', 'UF_SALES_NOTES', 'UF_GIFT'));
	...
	else
	{
		...
		$sectionIterator = CIBlockSection::GetList(array("LEFT_MARGIN"=>"ASC"), $filter, false, array('ID', 'IBLOCK_SECTION_ID', 'NAME', 'UF_DELIVERY', 'UF_DELIVERY_SROK', 'UF_PRICE_KOEF', 'UF_SALES_NOTES'));
		while ($section = $sectionIterator->Fetch())
		{
			$section["ID"] = (int)$section["ID"];
			$section["IBLOCK_SECTION_ID"] = (int)$section["IBLOCK_SECTION_ID"];
	...
	foreach ($arAvailGroups as &$value) {
		if($value['UF_DELIVERY_SROK']===""||$value['UF_DELIVERY_SROK']===null) {
			$tmp=$value;
			while($tmp['IBLOCK_SECTION_ID']>0) {
				$tmp = $arAvailGroups[$tmp['IBLOCK_SECTION_ID']];
				if($tmp['UF_DELIVERY_SROK']!==""&&$tmp['UF_DELIVERY_SROK']!==null) {
					$value['UF_DELIVERY_SROK'] = $tmp['UF_DELIVERY_SROK'];
					break;
				}
			}
		}
		if($value['UF_DELIVERY_SROK']===null) {
			$value['UF_DELIVERY_SROK'] = '';
		}
		if($value['UF_DELIVERY']===""||$value['UF_DELIVERY']===null) {
			$tmp=$value;
			while($tmp['IBLOCK_SECTION_ID']>0) {
				$tmp = $arAvailGroups[$tmp['IBLOCK_SECTION_ID']];
				if($tmp['UF_DELIVERY']!==""&&$tmp['UF_DELIVERY']!==null) {
					$value['UF_DELIVERY'] = $tmp['UF_DELIVERY'];
					break;
				}
			}
		}
		if($value['UF_DELIVERY']===null) {
			$value['UF_DELIVERY'] = '';
		}
		if($value['UF_DELIVERY_SROK']!=="") {
			if(!preg_match('@\d+(-\d+)?@', $value['UF_DELIVERY_SROK'], $m)) {
				$value['UF_DELIVERY_SROK'] = '';
			} else {
				$value['UF_DELIVERY_SROK'] = $m[0];
			}
		}
		...
		if(!$value['UF_SALES_NOTES']) {
			$tmp=$value;
			while($tmp['IBLOCK_SECTION_ID']>0) {
				$tmp = $arAvailGroups[$tmp['IBLOCK_SECTION_ID']];
				if($tmp['UF_SALES_NOTES']) {
					$value['UF_SALES_NOTES'] = $tmp['UF_SALES_NOTES'];
					break;
				}
			}
		}
...
$tmp = file_get_contents($fileName = dirname(__FILE__).'/price.txt');
...
if(($hour>=19)||($hour<8)) {
	$price = 'NIG';
} else if(($dayOfWeek==5&&$hour>=19)||($dayOfWeek==6)||($dayOfWeek==7&&$hour<8)) {
	$price = 'HOL';
} else {
	$price = 'GEN';
}
...
if($tmp != $price || true) {
	//var_dump($tmp, $price);
	require_once dirname(__FILE__).'/yandex_run_price.php';
}
		...
		while ($obElement = $res->GetNextElement())
		{
			...
			if ($arAcc["DETAIL_PICTURE"] > 0 || $arAcc["PREVIEW_PICTURE"] > 0)
			{
				...
				if( ($arItem['CATALOG_AVAILABLE'] != 'Y') ||
					($arAvailGroups[$arItem["IBLOCK_SECTION_ID"]]["UF_DELIVERY_SROK"]!=="" || $arAvailGroups[$arItem["IBLOCK_SECTION_ID"]]["UF_DELIVERY"]!=="")
				) {
					if($arAvailGroups[$arItem["IBLOCK_SECTION_ID"]]["UF_DELIVERY_FREE"]>0 && $arAvailGroups[$arItem["IBLOCK_SECTION_ID"]]["UF_DELIVERY_FREE"] < $minPrice) {
						$dp = 0;
					} else {
						$dp = $arAvailGroups[$arItem["IBLOCK_SECTION_ID"]]["UF_DELIVERY"]!=="" ? $arAvailGroups[$arItem["IBLOCK_SECTION_ID"]]["UF_DELIVERY"] : $local_delivery_cost;
					}
					if($arItem['CATALOG_AVAILABLE'] != 'Y') {
						$dd = '';
					} else {
						$dd = $arAvailGroups[$arItem["IBLOCK_SECTION_ID"]]["UF_DELIVERY_SROK"]!=="" ? $arAvailGroups[$arItem["IBLOCK_SECTION_ID"]]["UF_DELIVERY_SROK"] : $local_delivery_srok;
					}
					$strOfferYandex .= "<delivery>true</delivery>\n";
					$strOfferYandex .= "<delivery-options><option cost=\"$dp\" days=\"$dd\" /></delivery-options>\n";
				}
	...
	elseif ($arCatalog['CATALOG_TYPE'] == CCatalogSku::TYPE_PRODUCT || $arCatalog['CATALOG_TYPE'] == CCatalogSku::TYPE_FULL)
	{
		...
		$arSelect = array(
			"ID", "LID", "IBLOCK_ID", "IBLOCK_SECTION_ID", "NAME",
			"PREVIEW_PICTURE", "PREVIEW_TEXT", "PREVIEW_TEXT_TYPE", "DETAIL_PICTURE", "DETAIL_PAGE_URL",
			"CATALOG_AVAILABLE", "CATALOG_TYPE", "CATALOG_QUANTITY", "PROPERTY_BRAND.NAME"
		);

		$arFilter = array("IBLOCK_ID" => $IBLOCK_ID, "PROPERTY_NOMARKET"=>false);
		if (!$bAllSections && !empty($arSectionIDs))
		{
		...
		while ($obItem = $rsItems->GetNextElement())
		{
			...
			$arItem['CATALOG_AVAILABLE'] = $arItem['CATALOG_QUANTITY'] > 0 ? 'Y' : 'N';
			...
			{
				$pictNo = ($arItem["PREVIEW_PICTURE"] > 0 ? $arItem["PREVIEW_PICTURE"] : $arItem["DETAIL_PICTURE"]);
			...
			elseif ($arCatalog['CATALOG_TYPE'] == CCatalogSku::TYPE_FULL && $arItem['CATALOG_TYPE'] == Catalog\ProductTable::TYPE_PRODUCT)
			{
				...
				$available = ($arItem['CATALOG_AVAILABLE'] == 'Y');
				...
				$strOfferGoogle = '<item id="'.$arItem["ID"].'"'.$str_AVAILABLE.'>'."\n";
				$strOfferGoogle .= '<g:id>'.$arItem["ID"]."</g:id>\n";

				$strOfferGoogle .= '<link>'.$usedProtocol.$ar_iblock['SERVER_NAME'].htmlspecialcharsbx($arItem["~DETAIL_PAGE_URL"])."</link>\n";
				$strOfferGoogle .= '<title>'.yandex_text2xml_utf8($arItem["~NAME"], true)."</title>\n";
				$strOfferGoogle .= '<g:price>'.$minPrice." RUB</g:price>\n";
				$strOfferGoogle .= '<Type></Type>';
				$strOfferGoogle .= '<g:description>'.yandex_text2xml_utf8(strip_tags($arItem["~PREVIEW_TEXT"]), true)."</g:description>\n";
				$strOfferGoogle .= '<g:availability>'.($available?'in stock':'out of stock')."</g:availability>\n";
				//var_dump($arItem);
				$strOfferGoogle .= '<g:brand>'.$arItem["PROPERTY_BRAND_NAME"]."</g:brand>\n";
				//<g:mpn></g:mpn>
				$strOfferGoogle .= "<g:condition>NEW</g:condition>\n";
				if($arItem['YANDEX_PICT']) {
					$strOfferGoogle .= '<g:image_link>'.$arItem['YANDEX_PICT']."</g:image_link>\n";
				}
				$strOfferGoogle .= "</item>\n";
					...
					case 'description':
						$strValue =
							...
							yandex_text2xml(TruncateText(
								...
								255), true).
							"</description>\n";
							if( ($arItem['CATALOG_AVAILABLE'] == 'Y') && $arAvailGroups[$arItem["IBLOCK_SECTION_ID"]]["UF_SALES_NOTES"]) {
								$strValue .= "<sales_notes>".yandex_text2xml($arAvailGroups[$arItem["IBLOCK_SECTION_ID"]]["UF_SALES_NOTES"])."</sales_notes>\n";
							}
						break;
					case 'param':
						if ($parametricFieldsExist)
	...
	fwrite($fp, "<delivery-options><option cost=\"$local_delivery_cost\" days=\"$local_delivery_srok\" /></delivery-options>\n");
	...
	$arAllGifts = array();
	foreach($arGifts as $arTmpGift) {
		$arAllGifts = array_merge($arAllGifts, $arTmpGift);
	}
	$arAllGifts = array_unique($arAllGifts);
	if($arAllGifts) {
		$arGiftNotProduct = array_diff($arAllGifts, $arProducts);
		if($arGiftNotProduct) {
			unset($arFilter['SECTION_ID']);
			$arFilter['ID'] = $arGiftNotProduct;
			fwrite($fp, "<gifts>\n");
			$arGiftNotProduct = array();
			$rsItems = CIBlockElement::GetList(array('ID' => 'ASC'), $arFilter, false, false, $arSelect);
			while($arItem = $rsItems->GetNext()) {
				...
				fwrite($fp, "<gift id=\"".$arItem["ID"]."\">\n");
				fwrite($fp, "<name>".yandex_text2xml($arItem["~NAME"], true)."</name>\n");
				if($arItem['YANDEX_PICT']) {
					fwrite($fp, "<picture>".$arItem['YANDEX_PICT']."</picture>\n");
				}
				fwrite($fp, "</gift>\n");
				$arGiftNotProduct[]=$arItem["ID"];
			}
		...
		fwrite($fp, "<promos>\n");
		foreach($arGifts as $cat=>$tmpArGifts) {
			fwrite($fp, "<promo id=\"c".$cat."\" type=\"gift with purchase\">\n");
			fwrite($fp, "<purchase>\n");
			fwrite($fp, "<product category-id=\"".$cat."\"/>\n");
			fwrite($fp, "</purchase>\n");
			fwrite($fp, "<promo-gifts>\n");
			foreach($tmpArGifts as $gift) {
				if(in_array($gift, $arProducts)) {
					fwrite($fp, "<promo-gift offer-id=\"".$gift."\"/>\n");
				} else if(in_array($gift, $arGiftNotProduct)) {
					fwrite($fp, "<promo-gift gift-id=\"".$gift."\"/>\n");
				}
			}
			fwrite($fp, "</promo-gifts>\n");
			fwrite($fp, "</promo>\n");
	...
	rename($_SERVER["DOCUMENT_ROOT"].str_replace('yandex', 'google', $SETUP_FILE_NAME).'-tmp', $_SERVER["DOCUMENT_ROOT"].str_replace('yandex', 'google', $SETUP_FILE_NAME));
	rename($_SERVER["DOCUMENT_ROOT"].$SETUP_FILE_NAME.'-tmp', $_SERVER["DOCUMENT_ROOT"].$SETUP_FILE_NAME);
}
