...
if (!$dbHblock->Fetch())
{
	$data = array(
		'NAME' => 'ColorReference',
		'TABLE_NAME' => 'prostroy_color_reference',
	);

	$result = HL\HighloadBlockTable::add($data);
	$ID = $result->getId();

	$_SESSION['GORPO_HBLOCK_COLOR_ID'] = $ID;
...
$dbHblock = HL\HighloadBlockTable::getList(
	array(
		'filter' => array('NAME' => 'BrandReferenceStroy')
	)
);
if (!$dbHblock->Fetch())
{
	$data = array(
		'NAME' => 'BrandReferenceStroy',
		'TABLE_NAME' => 'prostroy_brand_reference',
	);

	$result = HL\HighloadBlockTable::add($data);
	$ID = $result->getId();

	$_SESSION['GORPO_HBLOCK_BRAND_ID'] = $ID;
