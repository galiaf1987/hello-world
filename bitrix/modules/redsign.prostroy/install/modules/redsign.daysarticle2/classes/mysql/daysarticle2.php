			...
			if(is_array($arrOffersID) && count($arrOffersID)>0)
			{
				...
				foreach($arrLIDs as $LID)
				{
					foreach($arrOffersID as $PRODUCT_ID)
					{
						$arFieldsDiscount = array(
							'SITE_ID' => $LID,
							'ACTIVE' => 'Y',
							'NAME' => GetMessage('DISCOUNT_NAME').' | Product ID = '.$arFields['ELEMENT_ID'].' [SKU ID = '.$PRODUCT_ID.']',
							'COUPON' => '',
							'SORT' => 100,
							'MAX_DISCOUNT' => 0,
				...
				foreach($arrLIDs as $LID)
				{
					$arFieldsDiscount = array(
						'SITE_ID' => $LID,
						'ACTIVE' => 'Y',
						'NAME' => GetMessage('DISCOUNT_NAME').' | Product ID = '.$arFields['ELEMENT_ID'],
						'COUPON' => '',
						'SORT' => 100,
						'MAX_DISCOUNT' => 0,
						'VALUE_TYPE' => $arFields['VALUE_TYPE'],
