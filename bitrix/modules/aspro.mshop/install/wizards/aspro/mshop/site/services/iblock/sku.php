...
$iblockTYPE = "aspro_mshop_catalog";
...
if(WIZARD_INSTALL_DEMO_DATA){
	if(!$iblockID){
		...
		if ($_SESSION["WIZARD_MSHOP_CATALOG_IBLOCK_ID"]) {
			$ID_SKU = CCatalog::LinkSKUIBlock($_SESSION["WIZARD_MSHOP_CATALOG_IBLOCK_ID"], $iblockID);
			$rsCatalogs = CCatalog::GetList(array(), array('IBLOCK_ID' => $iblockID), false, false, array('IBLOCK_ID'));
			if($arCatalog = $rsCatalogs->Fetch()){
				CCatalog::Update($iblockID, array('PRODUCT_IBLOCK_ID' => $_SESSION["WIZARD_MSHOP_CATALOG_IBLOCK_ID"], 'SKU_PROPERTY_ID' => $ID_SKU));
			}
			else{
				CCatalog::Add(array('IBLOCK_ID' => $iblockID, 'PRODUCT_IBLOCK_ID' => $_SESSION["WIZARD_MSHOP_CATALOG_IBLOCK_ID"], 'SKU_PROPERTY_ID' => $ID_SKU));
			}		
		}
	}
	else{
	...
	if ($_SESSION["WIZARD_MSHOP_CATALOG_IBLOCK_ID"]) {
		//create facet index
		$index = \Bitrix\Iblock\PropertyIndex\Manager::createIndexer($_SESSION["WIZARD_MSHOP_CATALOG_IBLOCK_ID"]);
		$index->startIndex();
		$index->continueIndex(0);
		$index->endIndex();

		unset($_SESSION["WIZARD_MSHOP_CATALOG_IBLOCK_ID"]);
	}
