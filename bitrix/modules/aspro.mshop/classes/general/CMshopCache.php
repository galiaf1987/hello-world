...
if(!class_exists("CMshopCache")){
	class CMshopCache {
				...
				if($arGroupBy === array()){
					...
					if($bGetDetailPageUrlsArray){
						...
						foreach($arByIBlock as $IBLOCK_ID => $arIB){
							...
							if($arSectionIDs){
								$arSections = CMshopCache::CIBlockSection_GetList(array("CACHE" => array("TAG" => CMshopCache::GetIBlockCacheTag($IBLOCK_ID), "MULTI" => "N", "GROUP" => array("ID"))), array("ID" => $arSectionIDs), false, array("ID", "CODE", "EXTERNAL_ID", "IBLOCK_ID"));
							}
							foreach($arIB as $SECTION_ID => $arIDs){
								if($arIDs){
									$rsElements = CIBlockElement::GetList(array(), array("ID" => $arIDs), false, false, array("ID", "DETAIL_PAGE_URL", "CANONICAL_PAGE_URL"));
									$rsElements->SetUrlTemplates(CMshopCache::$arIBlocksInfo[$IBLOCK_ID]["DETAIL_PAGE_URL"]);
		...
		function CSaleBasket_GetList($arOrder = array("SORT" => "ASC"), $arFilter = array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = array(), $cacheTag = "", $cacheTime = 36000000, $cachePath = ""){
			...
			if(!strlen($cachePath)){
				$cachePath = "/CMshopCache/sale/CSaleBasket_GetList/".$cacheTag."/";
			}
			$obCache = new CPHPCache();
		...
		function CCatalogStore_GetList($arOrder = array("SORT" => "ASC"), $arFilter = array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = array(), $cacheTag = "", $cacheTime = 36000000, $cachePath = ""){
			...
			if(!strlen($cachePath)){
				$cachePath = "/CMshopCache/catalog/CCatalogStore_GetList/".$cacheTag."/";
			}
			$obCache = new CPHPCache();
		...
		function CForumMessage_GetListEx($arOrder = array("SORT" => "ASC"), $arFilter = array(), $arGroupBy = false, $iNum = 0, $arSelectFields = array(), $cacheTag = "", $cacheTime = 36000000, $cachePath = ""){
			...
			if(!strlen($cachePath)){
				$cachePath = "/CMshopCache/forum/CForumMessage_GetListEx/".$cacheTag."/";
			}
			$obCache = new CPHPCache();
		...
		private function _InitCacheParams($moduleName, $functionName, $arCache){
			...
			if(!strlen($cachePath)){
				$cachePath = "/CMshopCache/".$moduleName."/".$functionName."/".$cacheTag."/";
			}
		...
		function GetIBlockCacheTag($IBLOCK_ID){
			if(!$IBLOCK_ID){
				...
				return @CMshopCache::$arIBlocksInfo[$IBLOCK_ID]["CODE"].$IBLOCK_ID;
			}
		}
		...
		function ClearTagIBlockElement($arFields){
			...
			if($arFields["IBLOCK_ID"]){
				$CACHE_MANAGER->ClearByTag(CMshopCache::GetIBlockCacheTag($arFields["IBLOCK_ID"]));
			}
		}
		...
		function ClearTagIBlockSectionBeforeDelete($ID){
			global $CACHE_MANAGER;
			if($ID > 0){
				if($IBLOCK_ID = CMshopCache::CIBlockSection_GetList(array("CACHE" => array("MULTI" => "N")), array("ID" => $ID), false, array("IBLOCK_ID"), true)){
					$CACHE_MANAGER->ClearByTag(CMshopCache::GetIBlockCacheTag($IBLOCK_ID));
				}
			}
		}
	}
	...
	// initialize CMshopCache::$arIBlocks array
	if(CMshopCache::$arIBlocks === NULL){
		$arIBlocksTmp = CMshopCache::CIBlock_GetList(array("CACHE" => array("TAG" => "iblocks")), array("ACTIVE" => "Y", "CHECK_PERMISSIONS" => "N"));
		CMshopCache::$arIBlocks = CMshopCache::GroupArrayBy($arIBlocksTmp, array("GROUP" => array("LID", "IBLOCK_TYPE_ID", "CODE"), "MULTI" => "Y", "RESULT" => array("ID")));
		CMshopCache::$arIBlocksInfo = CMshopCache::GroupArrayBy($arIBlocksTmp, array("GROUP" => array("ID")));
	}
}
?>