<?
if(!defined('MSHOP_MODULE_ID')){
	define('MSHOP_MODULE_ID', 'aspro.mshop');
}
...
class CMShop{
	...
	const moduleID		= MSHOP_MODULE_ID;
    const wizardID		= "aspro:mshop";
	const devMode 		= false;
	...
	function ShowLogo(){
		...
		$print_img = ($TEMPLATE_OPTIONS["LOGO_IMAGE"]["CURRENT_VALUE"] !== serialize(array()) ? $TEMPLATE_OPTIONS["LOGO_IMAGE"]["CURRENT_IMG"] : str_replace('//', '/', SITE_DIR."include/logo_color.png"));
		?>
		<a href="<?=SITE_DIR?>"><img src="<?=$TEMPLATE_OPTIONS["LOGO_IMAGE"]["CURRENT_IMG"]?>" alt="<?=$arSite["SITE_NAME"]?>" title="<?=$arSite["SITE_NAME"]?>" /></a>
		<a href="<?=SITE_DIR?>" class="print_img"><img src="<?=$print_img;?>" alt="<?=$arSite["SITE_NAME"]?>" title="<?=$arSite["SITE_NAME"]?>" /></a>
		<?
	}
	...
	function getViewedProducts($userID = false, $siteID = false){
		...
		$localKey = 'MARKET_VIEWED_ITEMS_'.$siteID;
	...
	function GetQuantityArray($totalCount, $arItemIDs = array(), $useStoreClick="N"){
		...
		if($arQuantityOptions === NULL){
			$arQuantityOptions = array(
				"USE_WORD_EXPRESSION" => COption::GetOptionString("aspro.mshop", "USE_WORD_EXPRESSION", "Y", SITE_ID),
				...
				"EXPRESSION_FOR_MIN" => COption::GetOptionString("aspro.mshop", "EXPRESSION_FOR_MIN", GetMessage("EXPRESSION_FOR_MIN_DEFAULT"), SITE_ID),
				"EXPRESSION_FOR_MID" => COption::GetOptionString("aspro.mshop", "EXPRESSION_FOR_MID", GetMessage("EXPRESSION_FOR_MID_DEFAULT"), SITE_ID),
				"EXPRESSION_FOR_MAX" => COption::GetOptionString("aspro.mshop", "EXPRESSION_FOR_MAX", GetMessage("EXPRESSION_FOR_MAX_DEFAULT"), SITE_ID),
				"EXPRESSION_FOR_EXISTS" => COption::GetOptionString("aspro.mshop", "EXPRESSION_FOR_EXISTS", GetMessage("EXPRESSION_FOR_EXISTS_DEFAULT"), SITE_ID),
				"EXPRESSION_FOR_NOTEXISTS" => COption::GetOptionString("aspro.mshop", "EXPRESSION_FOR_NOTEXISTS", GetMessage("EXPRESSION_FOR_NOTEXISTS_DEFAULT"), SITE_ID),
				"SHOW_QUANTITY_FOR_GROUPS" => (($tmp = COption::GetOptionString("aspro.mshop", "SHOW_QUANTITY_FOR_GROUPS", "", SITE_ID)) ? explode(",", $tmp) : array()),
				"SHOW_QUANTITY_COUNT_FOR_GROUPS" => (($tmp = COption::GetOptionString("aspro.mshop", "SHOW_QUANTITY_COUNT_FOR_GROUPS", "", SITE_ID)) ? explode(",", $tmp) : array()),
			);
	...
	function GetSKUPropsArray(&$arSkuProps, $iblock_id=0, $type_view="list", $hide_title_props="N", $group_iblock_id="N"){
		...
		if($iblock_id){
			//$arPropsSku=CMShop::GetPropertyViewType($iblock_id);
		...
		'<div class="bx_size_scroller_container"><div class="bx_size"><ul id="#ITEM#_prop_'.$arProp['ID'].'_list">';
			...
			'<span class="cnt"><span class="cnt_item" style="background-image:url(\''.$arOneValue['PICT']['SRC'].'\');" title="'.$arProp['NAME'].': '.$arOneValue['NAME'].'"></span></span></li>';
								}
								else
								{
									$templateRow .= '<li data-treevalue="'.$arProp['ID'].'_'.$arOneValue['ID'].'" data-showtype="li" data-onevalue="'.$arOneValue['ID'].'" title="'.$arProp['NAME'].': '.$arOneValue['NAME'].'"><i></i><span class="cnt">'.$arOneValue['NAME'].'</span></li>';
	...
	'<div class="bx_size_scroller_container"><div class="bx_size"><ul id="#ITEM#_prop_'.$arProp['ID'].'_list">';
					foreach ($arProp['VALUES'] as $arOneValue){
						if($arOneValue['ID']>0){
							$arOneValue['NAME'] = htmlspecialcharsbx($arOneValue['NAME']);
							$templateRow .= '<li data-treevalue="'.$arProp['ID'].'_'.$arOneValue['ID'].'" data-showtype="li" data-onevalue="'.$arOneValue['ID'].'" title="'.$arProp['NAME'].': '.$arOneValue['NAME'].'" style=""><i></i><span class="cnt">'.$arOneValue['NAME'].'</span></li>';
						}
					}
					$templateRow .= '</ul></div>'.
	'</div></div>';
	...
	'<div class="bx_scu_scroller_container"><div class="bx_scu"><ul id="#ITEM#_prop_'.$arProp['ID'].'_list">';
					foreach ($arProp['VALUES'] as $arOneValue){
						if($arOneValue['ID']>0){
							$arOneValue['NAME'] = htmlspecialcharsbx($arOneValue['NAME']);
							$templateRow .= '<li data-treevalue="'.$arProp['ID'].'_'.$arOneValue['ID'].'" data-showtype="li" data-onevalue="'.$arOneValue['ID'].'" style=""><i title="'.$arProp['NAME'].': '.$arOneValue['NAME'].'"></i>'.
	'<span class="cnt"><span class="cnt_item" style="background-image:url(\''.$arOneValue['PICT']['SRC'].'\');" title="'.$arProp['NAME'].': '.$arOneValue['NAME'].'"></span></span></li>';
						}
					}
					$templateRow .= '</ul></div>'.
	...
	function GetAddToBasketArray(&$arItem, $totalCount = 0, $defaultCount = 1, $basketUrl = '', $bDetail = false, $arItemIDs = array(), $class_btn = "small", $arParams=array()){
		...
		if($arAddToBasketOptions === NULL){
			$arAddToBasketOptions = array(
				"SHOW_BASKET_ONADDTOCART" => COption::GetOptionString("aspro.mshop", "SHOW_BASKET_ONADDTOCART", "Y", SITE_ID) == "Y",
				"USE_PRODUCT_QUANTITY_LIST" => COption::GetOptionString("aspro.mshop", "USE_PRODUCT_QUANTITY_LIST", "Y", SITE_ID) == "Y",
				"USE_PRODUCT_QUANTITY_DETAIL" => COption::GetOptionString("aspro.mshop", "USE_PRODUCT_QUANTITY_DETAIL", "Y", SITE_ID) == "Y",
				"BUYNOPRICEGGOODS" => COption::GetOptionString("aspro.mshop", "BUYNOPRICEGGOODS", "NOTHING", SITE_ID),
		...
		if($arItem["OFFERS"]){
			if(!$bDetail && $arItem["OFFERS_MORE"] != "Y" && $TEMPLATE_OPTIONS["TYPE_SKU"]["CURRENT_VALUE"] != "TYPE_2"){
				...
				$buttonHTML = '<span class="button '.$class_btn.' read_more to-cart" id="'.$arItemIDs['BUY_LINK'].'" data-offers="Y" data-iblockID="'.$arItem["IBLOCK_ID"].'" data-item="'.$arItem["ID"].'"><i></i><span>'.$buttonText[0].'</span></span><a rel="nofollow" href="'.$basketUrl.'" id="'.$arItemIDs['BASKET_LINK'].'" class="'.$class_btn.' in-cart button" data-item="'.$arItem["ID"].'"  style="display:none;"><i></i><span>'.$buttonText[1].'</span></a>';
			}
			elseif(($bDetail && $arItem["FRONT_CATALOG"] == "Y") || $arItem["OFFERS_MORE"]=="Y" || $TEMPLATE_OPTIONS["TYPE_SKU"]["CURRENT_VALUE"] == "TYPE_2"){
				...
				$buttonHTML = '<a class="button basket read_more" rel="nofollow" href="'.$arItem["DETAIL_PAGE_URL"].'" data-item="'.$arItem["ID"].'">'.$buttonText[0].'</a>';
			}
		}
		else{
				...
				if($totalCount > 0){
					...
					if((isset($arItem["CAN_BUY"]) && $arItem["CAN_BUY"]) || (isset($arItem["MIN_PRICE"]) && $arItem["MIN_PRICE"]["CAN_BUY"] == "Y")){
						if($bDetail && $arItem["FRONT_CATALOG"] == "Y"){
							...
							$buttonHTML = '<a class="button basket read_more" rel="nofollow" href="'.$arItem["DETAIL_PAGE_URL"].$rid.'" data-item="'.$arItem["ID"].'">'.$buttonText[0].'</a>';
						}
						else{
							...
							$buttonACTION = 'ADD';
							$buttonText = array(GetMessage('EXPRESSION_ADDTOBASKET_BUTTON_DEFAULT'), GetMessage('EXPRESSION_ADDEDTOBASKET_BUTTON_DEFAULT'));
							$buttonHTML = '<span class="'.$class_btn.' to-cart button" data-item="'.$arItem["ID"].'" data-float_ratio="'.$float_ratio.'" data-ratio="'.$ratio.'" data-bakset_div="bx_basket_div_'.$arItem["ID"].'" data-props="'.$arItemProps.'" data-part_props="'.$partProp.'" data-add_props="'.$addProp.'"  data-empty_props="'.$emptyProp.'" data-offers="'.$arItem["IS_OFFER"].'" data-iblockID="'.$arItem["IBLOCK_ID"].'"  data-quantity="'.$quantity.'"><i></i><span>'.$buttonText[0].'</span></span><a rel="nofollow" href="'.$basketUrl.'" class="'.$class_btn.' in-cart button" data-item="'.$arItem["ID"].'"  style="display:none;"><i></i><span>'.$buttonText[1].'</span></a>';
						}
					}
					elseif($arItem["CATALOG_SUBSCRIBE"] == "Y"){
						...
						$buttonHTML = '<span class="'.$class_btn.' transparent to-subscribe'.(!$bUserAuthorized ? ' auth' : '').' button" rel="nofollow" href="'.$arItem["SUBSCRIBE_URL"].'" data-item="'.$arItem["ID"].'"  alt="'.$arItem["NAME"].'"><i></i><span>'.$buttonText[0].'</span></span><span class="'.$class_btn.' transparent in-subscribe button" rel="nofollow" style="display:none;" data-item="'.$arItem["ID"].'" alt="'.$arItem["NAME"].'"><i></i><span>'.$buttonText[1].'</span></span>';
					}
				}
				else{
					...
					if($bDetail && $arItem["FRONT_CATALOG"] == "Y"){
						...
						$buttonACTION = $arParams['IS_SUBSCRIBE_LIST'] === 'Y' ? 'SUBSCRIBE' : $arAddToBasketOptions["BUYMISSINGGOODS"];
						if($buttonACTION == "ADD" /*|| $arItem["CAN_BUY"]*/){
							if($arItem["CAN_BUY"]){
								$buttonText = array(GetMessage('EXPRESSION_ADDTOBASKET_BUTTON_DEFAULT'), GetMessage('EXPRESSION_ADDEDTOBASKET_BUTTON_DEFAULT'));
								$buttonHTML = '<span class="'.$class_btn.' to-cart button" data-item="'.$arItem["ID"].'" data-float_ratio="'.$float_ratio.'" data-ratio="'.$ratio.'" data-bakset_div="bx_basket_div_'.$arItem["ID"].'" data-props="'.$arItemProps.'" data-part_props="'.$partProp.'" data-add_props="'.$addProp.'"  data-empty_props="'.$emptyProp.'" data-offers="'.$arItem["IS_OFFER"].'" data-iblockID="'.$arItem["IBLOCK_ID"].'" data-quantity="'.$quantity.'"><i></i><span>'.$buttonText[0].'</span></span><a rel="nofollow" href="'.$basketUrl.'" class="'.$class_btn.' in-cart button" data-item="'.$arItem["ID"].'"  style="display:none;"><i></i><span>'.$buttonText[1].'</span></a>';
							}else{
								if($buttonACTION == "SUBSCRIBE" && $arItem["CATALOG_SUBSCRIBE"] == "Y"){
									$buttonText = array($arAddToBasketOptions['EXPRESSION_SUBSCRIBE_BUTTON'], $arAddToBasketOptions['EXPRESSION_SUBSCRIBED_BUTTON']);
									$buttonHTML = '<span class="'.$class_btn.' transparent to-subscribe'.(!$bUserAuthorized ? ' auth' : '').' button" rel="nofollow" href="'.$arItem["SUBSCRIBE_URL"].'" data-item="'.$arItem["ID"].'"  alt="'.$arItem["NAME"].'"><i></i><span>'.$buttonText[0].'</span></span><span class="'.$class_btn.' transparent in-subscribe button" rel="nofollow" style="display:none;" data-item="'.$arItem["ID"].'" alt="'.$arItem["NAME"].'"><i></i><span>'.$buttonText[1].'</span></span>';
								}else{
									$buttonText = array($arAddToBasketOptions['EXPRESSION_ORDER_BUTTON']);
									$buttonHTML = '<span class="'.$class_btn.' to-order button transparent" data-name="'.$arItem["NAME"].'" data-item="'.$arItem["ID"].'"><i></i><span>'.$buttonText[0].'</span></span>';
						...
						elseif($buttonACTION == "SUBSCRIBE" && $arItem["CATALOG_SUBSCRIBE"] == "Y"){
							$buttonText = array($arAddToBasketOptions['EXPRESSION_SUBSCRIBE_BUTTON'], $arAddToBasketOptions['EXPRESSION_SUBSCRIBED_BUTTON']);
							$buttonHTML = '<span class="'.$class_btn.' transparent to-subscribe'.(!$bUserAuthorized ? ' auth' : '').' button" rel="nofollow" href="'.$arItem["SUBSCRIBE_URL"].'" data-item="'.$arItem["ID"].'"  alt="'.$arItem["NAME"].'"><i></i><span>'.$buttonText[0].'</span></span><span class="'.$class_btn.' transparent in-subscribe button" rel="nofollow" style="display:none;" data-item="'.$arItem["ID"].'" alt="'.$arItem["NAME"].'"><i></i><span>'.$buttonText[1].'</span></span>';
						}
						elseif($buttonACTION == "ORDER"){
							$buttonText = array($arAddToBasketOptions['EXPRESSION_ORDER_BUTTON']);
							$buttonHTML = '<span class="'.$class_btn.' to-order button transparent" data-name="'.$arItem["NAME"].'" data-item="'.$arItem["ID"].'"><i></i><span>'.$buttonText[0].'</span></span>';
							if($arAddToBasketOptions['EXPRESSION_ORDER_TEXT']){
								$buttonHTML .='<div class="more_text">'.$arAddToBasketOptions['EXPRESSION_ORDER_TEXT'].'</div>';
	...
	function GetTemplateOptions($siteID){
		...
		foreach($arComponentOptions as $value){
			...
			elseif($value['ID'] === 'LOGO_IMAGE' || $value['ID'] === 'FAVICON_IMAGE' || $value['ID'] === 'APPLE_TOUCH_ICON_57_IMAGE' || $value['ID'] === 'APPLE_TOUCH_ICON_72_IMAGE'){
				...
				if($fileID){
					...
					elseif($value['ID'] === 'APPLE_TOUCH_ICON_57_IMAGE'){
						$arTemplateOptions[$value["ID"]]["CURRENT_IMG"] = str_replace('//', '/', SITE_DIR.'/include/favicon_57.png');
					}
					elseif($value['ID'] === 'APPLE_TOUCH_ICON_72_IMAGE'){
						$arTemplateOptions[$value["ID"]]["CURRENT_IMG"] = str_replace('//', '/', SITE_DIR.'/include/favicon_72.png');
					}
				}
	...
	static function getModuleOptionsList($onlyTemplate = false){
		...
		$arTemplateOptions[] = array(
			"ID" => "BANNER_WIDTH",
			"NAME" => GetMessage("BANNER_WIDTH"),
			"TYPES" => array("COMPONENT"=>"TEXT", "MODULE" => "SELECTBOX"),
			"VALUES" => array(
				array("VALUE" => "NARROW", "COMPONENT_VALUE" => "", "NAME" => GetMessage("BANNER_WIDTH_NARROW")),
				array("VALUE" => "AVERAGE", "COMPONENT_VALUE" => "", "NAME" => GetMessage("BANNER_WIDTH_MIDDLE")),
				array("VALUE" => "AUTO", "COMPONENT_VALUE" => "", "NAME" => GetMessage("BANNER_WIDTH_AUTO")),
			),
			"DEFAULT" => "AUTO",
		);
		...
		if(!$onlyTemplate){
			...
			$arTemplateOptions[] = array(
				"ID" => "APPLE_TOUCH_ICON_57_IMAGE",
				"NAME" => GetMessage("APPLE_TOUCH_ICON_57_IMAGE"),
				"TYPES" => array("COMPONENT" => "TEXT", "MODULE" => "file"),
				"DEFAULT" => serialize(array()),
			);
			$arTemplateOptions[] = array(
				"ID" => "APPLE_TOUCH_ICON_72_IMAGE",
				"NAME" => GetMessage("APPLE_TOUCH_ICON_72_IMAGE"),
				"TYPES" => array("COMPONENT" => "TEXT", "MODULE" => "file"),
				"DEFAULT" => serialize(array()),
			);
		}
		...
		$arTemplateOptions = array_merge($arTemplateOptions, array(
				array(
					...
					"TYPES" => array("COMPONENT"=>"TEXT", "MODULE" => "SELECTBOX"),
					"VALUES" => array(
						array("VALUE" => "TYPE_1", "COMPONENT_VALUE" => "", "NAME" => GetMessage("TYPE_1"), "IMG" => "include/logo_color.png", "MENU_TYPE" => "type_1", "HEAD_COLOR" => "white", "MENU_COLOR" => ""),
						array("VALUE" => "TYPE_2", "COMPONENT_VALUE" => "", "NAME" => GetMessage("TYPE_2"), "IMG" => "include/logo_white.png", "MENU_TYPE" => "type_1", "HEAD_COLOR" => "colored", "MENU_COLOR" => ""),
						array("VALUE" => "TYPE_3", "COMPONENT_VALUE" => "", "NAME" => GetMessage("TYPE_3"), "IMG" => "include/logo_color.png", "MENU_TYPE" => "type_2", "HEAD_COLOR" => "white", "MENU_COLOR" => "white"),
						array("VALUE" => "TYPE_4", "COMPONENT_VALUE" => "", "NAME" => GetMessage("TYPE_4"), "IMG" => "include/logo_color.png", "MENU_TYPE" => "type_2", "HEAD_COLOR" => "white", "MENU_COLOR" => "dark"),
						array("VALUE" => "TYPE_5", "COMPONENT_VALUE" => "", "NAME" => GetMessage("TYPE_5"), "IMG" => "include/logo_color.png", "MENU_TYPE" => "type_2", "HEAD_COLOR" => "white", "MENU_COLOR" => "colored"),
						array("VALUE" => "TYPE_6", "COMPONENT_VALUE" => "", "NAME" => GetMessage("TYPE_6"), "IMG" => "include/logo_white.png", "MENU_TYPE" => "type_2", "HEAD_COLOR" => "colored", "MENU_COLOR" => "white"),
						array("VALUE" => "TYPE_7", "COMPONENT_VALUE" => "", "NAME" => GetMessage("TYPE_7"), "IMG" => "include/logo_white.png", "MENU_TYPE" => "type_2", "HEAD_COLOR" => "colored", "MENU_COLOR" => "dark"),
						array("VALUE" => "TYPE_8", "COMPONENT_VALUE" => "", "NAME" => GetMessage("TYPE_8"), "IMG" => "include/logo_white.png", "MENU_TYPE" => "type_2", "HEAD_COLOR" => "colored", "MENU_COLOR" => "colored"),
					),
					"DEFAULT" => "TYPE_1",
				),
				array(
		...
		$arTemplateOptions = array_merge($arTemplateOptions, array(
				...
				array(
					...
					"TYPES" => array("COMPONENT" => "TEXT", "MODULE" => "SELECTBOX"),
					"VALUES" => array(
						array("VALUE" => "LOCAL", "COMPONENT_VALUE" => "", "NAME" => GetMessage("VIEWED_TYPE_LOCAL")),
						array("VALUE" => "BX", "COMPONENT_VALUE" => "", "NAME" => GetMessage("VIEWED_TYPE_BX")),
					),
					"DEFAULT" => "BX",
				),
				array(
				...
				array(
					...
					"TYPES" => array("COMPONENT"=>"TEXT", "MODULE" => "SELECTBOX"),
					"VALUES" => array(
						array("VALUE" => "NO", "COMPONENT_VALUE" => "", "NAME" => GetMessage("NO")),
						array("VALUE" => "LIGHT", "COMPONENT_VALUE" => "", "NAME" => GetMessage("LIGHT_STORES")),
						array("VALUE" => "DARK", "COMPONENT_VALUE" => "", "NAME" => GetMessage("DARK_STORES")),
					),
					"DEFAULT" => "LIGHT",
				),
				array(
		...
		$arModuleOptions[] = array("SHOW_BASKET_PRINT", GetMessage("SHOW_BASKET_PRINT"), "N", array("checkbox"));
		...
		$arModuleOptions[] = array("SHOW_MORE_ITEM_MENU_LINK", GetMessage("SHOW_MORE_ITEM_MENU_LINK"), "Y", array("checkbox"));
		$arModuleOptions[] = array("SHOW_CALLBACK", GetMessage("SHOW_CALLBACK"), "Y", array("checkbox"));
		...
		if(!$onlyTemplate){
			...
			$arModuleOptions[] = array(
				"BANNER_WIDTH",
				GetMessage("BANNER_WIDTH"),
				"AUTO",
				array(
					"selectbox",
					array(
						"NARROW" => GetMessage("BANNER_WIDTH_NARROW"),
						"AVERAGE" => GetMessage("BANNER_WIDTH_MIDDLE"),
			...
			$arModuleOptions[] = array(
				...
				array(
					...
					array(
						"NO" => GetMessage("NO"),
						"LIGHT" => GetMessage("LIGHT_STORES"),
						"DARK" => GetMessage("DARK_STORES"),
					),
				),
			);
			$arModuleOptions[] = array(
			...
			$arModuleOptions[] = array("ONECLICKBUY_PROPERTIES", GetMessage("ONECLICKBUY_PROPERTIES"), "FIO,PHONE,EMAIL,COMMENT", array("multiselectbox", array(), 10));
			$arModuleOptions[] = array("ONECLICKBUY_REQUIRED_PROPERTIES", GetMessage("ONECLICKBUY_REQUIRED_PROPERTIES"), "FIO,PHONE", array("multiselectbox", array(), 10));
			...
			$arModuleOptions[] = array("MIN_ORDER_PRICE_TEXT", GetMessage("MIN_ORDER_PRICE_TEXT"), GetMessage("MIN_ORDER_PRICE_TEXT_EXAMPLE"), array("textarea", "2", "50"));
			...
			// show search options
			//$arModuleOptions[] = GetMessage("SEARCH_OPTIONS");
	...
	function SetJSOptions(){
		...
		jQuery.extend(jQuery.validator.messages,{
			...
			minlength: '<?=GetMessage("VALIDATOR_MINLENGTH")?>',
			equalTo: '<?=GetMessage("VALIDATOR_EQUALTO")?>',
			captcha: '<?=GetMessage("VALIDATOR_CAPTCHA")?>',
		});
	...
	function __AdmSettingsSaveOption_EX($module_id, $arOption){
		...
		if(strpos($name, 'CUSTOM_COLOR_THEME') !== false){
			$val = str_replace('#', '', $val);
	...
	function __AdmSettingsDrawRow_EX($module_id, $Option, $siteID){
		...
		?>
			<?if($name === "ONECLICKBUY_PROPERTIES"):?>
				<tr>
					<td colspan="2" align="center">
						<div class="adm-info-message"><?=GetMessage("APPLY_PERSON_TYPE")?></div><div style="clear:both;"></div>
					</td>
				</tr>
			<?endif;?>
			<tr>
				...
				elseif($type[0]=="multiselectbox"):
					...
					?>
					<select size="<?=($type[2] ? $type[2] :5);?>" <?if(isset($arControllerOption[$Option[0]]))echo ' disabled title="'.GetMessage("MAIN_ADMIN_SET_CONTROLLER_ALT").'"';?> multiple name="<?echo htmlspecialcharsbx($Option[0])?>[]"<?=$disabled?>><?
						$count = count($arr_keys);
				...
				elseif($type[0]=='file'):
					...
					elseif(strpos($Option[0], 'APPLE_TOUCH_ICON_57_IMAGE') !== false) {
						$arOption['WIDTH'] = 57;
						$arOption['HEIGHT'] = 57;
					}
					elseif(strpos($Option[0], 'APPLE_TOUCH_ICON_72_IMAGE') !== false) {
						$arOption['WIDTH'] = 72;
						$arOption['HEIGHT'] = 72;
					}
	...
	static function InsertCounters(&$html){
		...
		if(defined("LOAD_COUNTERS")  && (strpos($APPLICATION->GetCurPage(),'personal')===false || strpos($APPLICATION->GetCurPage(),'auth')===false)){
			$counter = '';
...
';
	...
	function OnSaleComponentOrderOneStepPersonTypeHandler($arResult, &$arUserResult, $arParams){
		if(!$arUserResult["DELIVERY_LOCATION"]){
			...
			{
				$arUserResult["DELIVERY_LOCATION"]=$arProperties["DEFAULT_VALUE"];
			}
		}
	}
		...
		if(is_array($arShops)){
			foreach($arShops as $i => $arShop){
				if(isset($arShop['IBLOCK_ID'])){
					...
					$arShop['IMAGE'] = ($imageID ? CFile::ResizeImageGet($imageID, array('width' => 100, 'height' => 69), BX_RESIZE_IMAGE_EXACT) : '');
					$arShop['ADDRESS'] = $arShop['DISPLAY_PROPERTIES']['ADDRESS']['VALUE'];
					$arShop['ADDRESS'] = $arShop['TITLE'].((strlen($arShop['TITLE']) && strlen($arShop['ADDRESS'])) ? ',<br /> ' : '').$arShop['ADDRESS'];
					$arShop['PHONE'] = $arShop['DISPLAY_PROPERTIES']['PHONE']['VALUE'];
					$arShop['EMAIL'] = $arShop['DISPLAY_PROPERTIES']['EMAIL']['VALUE'];
					if(strtolower($arShop['DISPLAY_PROPERTIES']['SCHEDULE']['VALUE']['TYPE']) == 'html'){
						$arShop['SCHEDULE'] = htmlspecialchars_decode($arShop['DISPLAY_PROPERTIES']['SCHEDULE']['VALUE']['TEXT']);
										...
										<div class="schedule_phone_email">
											...
											<div class="phone_email">
												<?if(strlen($arShop["PHONE"])):?>
													<div class="phone"><a rel="nofollow" href="tel:<?=str_replace(" ", "", $arShop["PHONE"]);?>"><?=$arShop["PHONE"];?></a></div>
												<?endif;?>
	...
	function drawShopDetail($arShop, $arParams, $showMap="Y"){
		...
		if(is_array($arShop)){
			if(isset($arShop['IBLOCK_ID'])){
				...
				$arShop["TITLE"] = htmlspecialchars_decode($arShop["TITLE"]);
				$arShop["ADDRESS"] = htmlspecialchars_decode($arShop["ADDRESS"]);
				$arShop["ADDRESS"] = (strlen($arShop["TITLE"]) ? $arShop["TITLE"].',<br /> ' : '').$arShop["ADDRESS"];
				$arShop["DESCRIPTION"] = htmlspecialchars_decode($arShop['DESCRIPTION']);
				$arShop['SCHEDULE'] = htmlspecialchars_decode($arShop['SCHEDULE']);
					...
					<div class="store_description">
						...
						<?if(strlen($arShop['PHONE'])):?>
							<div class="store_property">
								...
								<div class="value"><a rel="nofollow" href="tel:<?=str_replace(" ", "", $arShop['PHONE']);?>"><?=$arShop['PHONE']?></a></div>
							</div>
						<?endif;?>
						<?if(strlen($arShop['EMAIL'])):?>
							<div class="store_property">
	...
	function getBasketAspro(){
		$arResult = array('BASKET' => array(), 'DELAY' => array(), 'SUBSCRIBE' => array(), 'COMPARE' => array());
		$bUseSubscribeManager = ($arSubscribeList = self::getUserSubscribeList()) !== false;

		if(CModule::IncludeModule('sale')){
			$dbRes = CSaleBasket::GetList(array(), array('FUSER_ID' => CSaleBasket::GetBasketUserID(), 'LID' => SITE_ID, 'ORDER_ID' => 'NULL'), false, false, array('ID', 'PRODUCT_ID', 'DELAY', 'SUBSCRIBE', 'CAN_BUY', 'TYPE', 'SET_PARENT_ID'));
			while($arItem = $dbRes->Fetch()){
				if(CSaleBasketHelper::isSetItem($arItem)){
					continue;
				}
				if($arItem['DELAY'] === 'N' && $arItem['CAN_BUY'] === 'Y' && $arItem['SUBSCRIBE'] === 'N'){
					$arResult['BASKET'][$arItem['PRODUCT_ID']] = $arItem['PRODUCT_ID'];
				}
				elseif($arItem['DELAY'] === 'Y' && $arItem['CAN_BUY'] === 'Y' && $arItem['SUBSCRIBE'] === 'N'){
					$arResult['DELAY'][$arItem['PRODUCT_ID']] = $arItem['PRODUCT_ID'];
				}
				elseif($arItem['SUBSCRIBE'] === 'Y' && !$bUseSubscribeManager){
					$arResult['SUBSCRIBE'][$arItem['PRODUCT_ID']] = $arItem['PRODUCT_ID'];
				}
			}
		}
		...
		if($bUseSubscribeManager && $arSubscribeList){
			foreach($arSubscribeList as $PRODUCT_ID => $arIDs){
				$arResult['SUBSCRIBE'][$PRODUCT_ID] = $PRODUCT_ID;
			}
		}
	...
	function checkVersionExt($template="main", $module="catalog", $default="N")
	{
	...
	function checkVersionModule($module="catalog", $testVersion="16.0.0")
	{
		if($info = CModule::CreateModuleObject($module))
		{
			if(CheckVersion($testVersion, $info->MODULE_VERSION))
