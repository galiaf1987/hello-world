...
class Offers
{
	...
	public function __construct($options = array())
	{
		...
		//Параметры профиля
		if($options['PROFILE'])
			$this->profile = (array)$options['PROFILE'];
	...
	protected function initElementList()
	{
		...
		foreach($this->iblockProducts as $iblock => $products) {
			...
			if(!empty($iblockElements) && !$this->hasErrors()) {
				...
				$this->modifyDisplayProperties($iblock, $propertyCode, $iblockElements);
	...
	protected function getIblockOffers($iblockId)
	{
		...
		if(
			...
			if(!empty($offersId)) {
				...
				$this->modifyDisplayProperties($catalog['IBLOCK_ID'], $propertyCode, $offers);
	...
	protected function modifyDisplayProperties($iblockID, $propCodes, &$result)
	{
		//$iblockParams = $this->storage['IBLOCK_PARAMS'][ $iblock ];
	...
	/** @deprecated  Allow max memory usage */
	protected
	function getPropertyList($iblock, $propertyCodes)
	{
		$propertyList = array();
		if(empty($propertyCodes))
			return $propertyList;
